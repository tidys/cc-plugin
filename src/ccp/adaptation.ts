...
class Builder {
  ...
  async getConfig(): Promise<BuilderOptions[]> {
    ...
    if (adaptation.Env.isPluginV2) {
      const buildCfgFile = Path.join(
        adaptation.Project.path,
        "local/builder.json"
      );
      if (Fs.existsSync(buildCfgFile)) {
        const buildData: {
          ...
          buildPath: string;
          platform: string;
        } = JSON.parse(Fs.readFileSync(buildCfgFile, "utf-8"));
      ...
      // @ts-ignore
      let cfg = await Editor.Profile.getConfig(
        "builder",
      ...
      Object.keys(cfg).forEach((key) => {
        ...
        if (item) {
          ...
          const outputPath = buildPath
            ? Path.join(buildPath, item.outputName)
            : item.buildPath;
          ret.push({
            buildPath: buildPath,
  ...
  get path(): string {
    if (adaptation.Env.isPluginV2) {
      ...
      if (adaptation.Env.isWin) {
        ...
        return Path.join(
          adaptation.CCEditor.path,
          "resources/3d/engine-native/simulator/Debug/"
        );
      }
    }
    return "";
...
class Dialog {
  ...
  ): Promise<Record<string, string | null | ArrayBuffer>> {
    if (adaptation.Env.isWeb) {
      return new Promise((resolve, reject) => {
...
export class Adaptation {
  ...
  public require(name: string): any {
    if (adaptation.Env.isPluginV2) {
      // @ts-ignore
      return Editor.require(
        `packages://${config.manifest!.name}/node_modules/${name}`
      );
    } else {
