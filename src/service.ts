...
export class CocosPluginService {
    ...
    public dealAssetDb(link: boolean = true) {
        ...
        if (dbDir) {
            if (!dbDir.path) {
                log.red(`asset db dir path is empty`);
                process.exit(1);
            }
            const fullpath = Path.join(this.context, dbDir.path);
            if (!FS.existsSync(fullpath)) {
                log.red(`asset db dir not exist: ${dbDir.path}`);
                process.exit(1);
            }
    ...
    private checkPluginName(name: string) {
        if (!name) {
            log.red("插件名字为空，请在cc-plugin.config.ts中填写有效的插件名字");
            process.exit(0);
        }
        ...
        if (/^[a-z][a-z0-9-_]{0,213}$/.test(name) === false) {
            log.red(`插件名字 ${name} 不合法，必须以小写字母开头，只能包含小写字母、数字、下划线、横线。`);
            process.exit(0);
        }
    }
